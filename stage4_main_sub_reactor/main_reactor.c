...
// 主 Reactor 回调定义：包含指向 MainReactor 的指针
typedef struct MainEvent {
    int fd;
    int epoll_fd;
    MainReactor *main_r;
} MainEvent;
...
void main_accept_cb(Event *ev) {
    // 强制转换为 MainEvent 或通过成员获取 MainReactor
    ...
    // 步骤 2：轮询选择一个 Sub-Reactor (Round-Robin 算法)
    ...
    printf("[->] [Main-Reactor] 采用 Round-Robin 算法将 conn_fd=%d 分发给 Sub-Reactor %d\n",
           conn_fd, target_sub->id);

    // 步骤 3 & 4：塞入 Sub-Reactor 队列并通过 write(eventfd) 唤醒！
...
int main_reactor_init(MainReactor *main_r, int port) {
    ...
    // 1. 初始化并启动所有 Sub-Reactors (子线程群)
    for (int i = 0; i < SUB_REACTOR_NUM; i++) {
        if (sub_reactor_init(&main_r->sub_reactors[i], i) < 0) {
    ...
    // 2. 创建 Main-Reactor 的 Socket 并监听
    ...
    listen_ev->sub = (SubReactor *)main_r; // 借助 sub 指针传递 MainReactor 指针
    ...
    printf("[阶段四] Main-Sub Multi-Reactor 启动，监听端口: %d, Sub-Reactors 数量: %d\n",
